...
Passing the input `(div (add x x) 2)`, the example outputs `x`. Also, passing the input `(div (add x x) x)`, the example outputs `2`. Despite of being very minimalistic framework, *Symbolverse* is Turing complete system. However, while it is possible to use *Symbolverse* for any abstract programming task, it is best suited for expression transformation and symbolic reasoning.
